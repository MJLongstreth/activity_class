...
acc_paths = glob.glob('data/flat/*acc_stream.csv')
gyro_paths = glob.glob('data/flat/*gyro_stream.csv')
magn_paths = glob.glob('data/flat/*magn_stream.csv')
...
acc_df[['delete', 'folder1','folder2', 'label', 'sensor_type', 'collection_date', 'file']] = acc_df['source'].str.split('/', expand=True)
gyro_df[['delete', 'folder1', 'folder2', 'label', 'sensor_type', 'collection_date', 'file']] = gyro_df['source'].str.split('/', expand=True)
magn_df[['delete', 'folder1', 'folder2', 'label', 'sensor_type', 'collection_date', 'file']] = magn_df['source'].str.split('/', expand=True)

# Clean up environment
del i,acc_dfs, gyro_dfs, magn_dfs

# Drop uneeded columns from each dataframe
acc_df = acc_df.drop(['source', 'delete', 'folder1', 'folder2', 'sensor_type', 'file', 'collection_date'], axis=1)
gyro_df = gyro_df.drop(['source', 'delete', 'folder1', 'folder2', 'sensor_type', 'file', 'collection_date'], axis=1)
magn_df = magn_df.drop(['source', 'delete', 'folder1', 'folder2','sensor_type', 'file', 'collection_date'], axis=1)
...
acc_df.to_csv('./data/merged/acc_data.csv')
gyro_df.to_csv('./data/merged/gyro_data.csv')
magn_df.to_csv('./data/merged/magn_data.csv')
acc_gyro_data.to_csv('./data/merged/acc_gyro_data.csv')
all_sensor_data.to_csv('./data/merged/all_sensor_data.csv')
